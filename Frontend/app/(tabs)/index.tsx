...
export default function HomeScreen() {
  ...
  const enhancedStatsData = [
    { icon: Shield, label: "Risk Level", value: stats.riskLevel, color: getRiskLevelColor(stats.riskLevel), gradient: getRiskLevelGradient(stats.riskLevel) },
    { icon: TrendingUp, label: "Trend", value: stats.improvementTrend, color: getTrendColor(stats.improvementTrend), gradient: getTrendGradient(stats.improvementTrend) },
    { icon: Activity, label: "Top Condition", value: stats.mostCommonCondition === 'None' ? 'Healthy' : stats.mostCommonCondition, color: COLORS.accent.cyan, gradient: COLORS.gradients.accent },
  ];
  ...
  function getRiskLevelGradient(riskLevel: string) {
    switch (riskLevel) {
      case 'Low': return COLORS.gradients.secondary;
      case 'Medium': return COLORS.gradients.warm;
      case 'High': return [COLORS.error, '#DC2626'];
        ...
        {stats.diseaseStats.length > 0 && (
          <Animated.View style={[
            ...
            <ThemedView style={styles.sectionHeader}>
              <ThemedText style={styles.sectionTitle}>Disease Analysis 📊</ThemedText>
              <ThemedText style={styles.sectionSubtitle}>Your detected conditions</ThemedText>
            </ThemedView>
            
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.diseaseStatsList}>
              {stats.diseaseStats.slice(0, 3).map((disease, index) => (
                <Animated.View
                  key={disease.name}
                  style={[
                    { opacity: scaleAnim },
                ...
                >
                  <TouchableOpacity style={styles.diseaseCard} activeOpacity={0.8}>
                    <LinearGradient
                      colors={[COLORS.primary[100], COLORS.primary[200]]}
                      style={styles.diseaseCardHeader}
                    >
                      <ThemedText style={styles.diseaseName}>{disease.name}</ThemedText>
                      <ThemedText style={styles.diseaseCount}>{disease.count} scans</ThemedText>
                    </LinearGradient>
                    
                    <ThemedView style={styles.diseaseCardContent}>
                      <ThemedView style={styles.diseaseStat}>
                      ...
                      <ThemedView style={styles.severityBreakdown}>
                        <ThemedText style={styles.severityLabel}>Severity:</ThemedText>
                        <ThemedView style={styles.severityBars}>
                          {Object.entries(disease.severityBreakdown).map(([severity, count]) => (
                            count > 0 && (
                              <ThemedView key={severity} style={styles.severityBar}>
                                <ThemedText style={styles.severityBarText}>{severity}</ThemedText>
                                <ThemedText style={styles.severityBarCount}>{count}</ThemedText>
                              </ThemedView>
                            )
                          ))}
                        </ThemedView>
